...
use moss_nv::{
    fan_curve::{hysteresis_curve::HysteresisCurve, linear_curve::LinearCurve, FanCurve}, fan_manager::{FanManager, FanMessage}, logger
};
...
async fn main() -> Result<()> {
    ...
    _tx_fan_manager.send(FanMessage::SetMode { 
        uuid: uuid.clone(),
        mode: moss_nv::fan_manager::FanMode::Manual 
    }).await.map_err(|_| {
        anyhow!("Fan manager send error")
    })?;
